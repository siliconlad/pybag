...
class Ros1MsgSchemaDecoder(SchemaDecoder):
    ...
    def _create_builtin_schemas(self) -> dict[str, Schema]:
        """Create schemas for built-in ROS1 message types."""
        builtin_schemas = {}

        # std_msgs/Time (ROS1 uses std_msgs instead of builtin_interfaces)
        builtin_schemas['std_msgs/Time'] = Schema(
            'std_msgs/Time',
            {
                'sec': SchemaField(Primitive('uint32'), None),
                'nsec': SchemaField(Primitive('uint32'), None)
            }
        )

        # std_msgs/Duration
        builtin_schemas['std_msgs/Duration'] = Schema(
            'std_msgs/Duration',
            {
                'sec': SchemaField(Primitive('int32'), None),
                'nsec': SchemaField(Primitive('int32'), None)
            }
        )
    ...
    def _parse_value(self, field_type: SchemaFieldType, raw_value: str) -> Any:
        ...
        if isinstance(field_type, String):
            if field_type.max_length is not None and len(raw_value) > field_type.max_length:
                raise Ros1MsgError('String default exceeds length')
        ...
        if isinstance(field_type, (Array, Sequence)):
            ...
            if not isinstance(values, list):
                raise Ros1MsgError('Array default must be a list')
    ...
    def _parse_field_type(self, field_raw_type: str, package_name: str) -> SchemaFieldType:
        ...
        if field_raw_type.startswith('string'):
            if not (match := re.match(r'string(?:<=(\d+))?$', field_raw_type)):
                raise Ros1MsgError(f'Invalid string field: {field_raw_type}')
        ...
        if field_raw_type.startswith('wstring'):
            if not (match := re.match(r'wstring(?:<=(\d+))?$', field_raw_type)):
                raise Ros1MsgError(f'Invalid wstring field: {field_raw_type}')
            length = int(match.group(1)) if match.group(1) else None
    ...
    def _parse_field(self, field: str, package_name: str) -> tuple[str, SchemaEntry]:
        ...
        if is_constant := ('=' in field_raw_name or (raw_default and raw_default.startswith('='))):
            ...
            if not field_raw_name.isupper():
                raise Ros1MsgError('Constant name must be uppercase')
            if not raw_default:
                raise Ros1MsgError('Constant value cannot be empty')
        else:
            if not re.match(r'[a-z][a-z0-9_]*$', field_raw_name):
                raise Ros1MsgError('Field name must be lowercase, alphanumeric or _')
        ...
        if raw_default is not None:
            ...
            if isinstance(schema_type, Complex):
                raise Ros1MsgError('Complex fields cannot have default values')
            if isinstance(schema_type, Array) and isinstance(schema_type.type, String):
                raise Ros1MsgError('Array of strings cannot have default values')
            if isinstance(schema_type, Sequence) and isinstance(schema_type.type, String):
                raise Ros1MsgError('Sequence of strings cannot have default values')
            default_value = self._parse_value(schema_type, raw_default)

        if is_constant:
            if default_value is None:
                raise Ros1MsgError('Constant must have a default value')
...
class Ros1MsgSchemaEncoder(SchemaEncoder):
    ...
    def _parse_annotation(self, annotation_type: Any) -> SchemaFieldType:
        ...
        if len(annotation_args) < 2:
            raise Ros1MsgError(f"Field is not correctly annotated.")
