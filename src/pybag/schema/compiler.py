...
# Schemas that can be inlined as a single struct.unpack_from call
# Maps schema name -> (format_string, size, field_names)
_INLINABLE_SCHEMAS: dict[str, tuple[str, int, list[str]]] = {}
...
def compile_schema(schema: Schema, sub_schemas: dict[str, Schema]) -> Callable[[MessageDecoder], type]:
    ...
    def create_dataclass_type(current: Schema) -> type:
        ...
        # Note: We avoid using __slots__ because dataclasses with default values
        # (including constants) have class-level attributes that conflict with slots
                ...
                if simple_info is not None:
                    ...
                    while idx < len(field_info):
                        ...
                        if len(inline_run_fields) == 1:
                            lines.append(f"{_TAB}{inline_run_var_names} = struct.unpack_from(fmt_prefix + '{inline_fmt_str}', _view, _data._position)[0]")
                        else:
                            lines.append(f"{_TAB}{inline_run_var_names} = struct.unpack_from(fmt_prefix + '{inline_fmt_str}', _view, _data._position)")
                        lines.append(f"{_TAB}_data._position += {inline_total_size}")
