...
def test_parse_complex_same_package_field():
    schema_text = "Sub other\n"
    schema = SchemaRecord(
        id=1,
        name="pkg/msg/Main",
        encoding="ros2msg",
    ...
    field = ros2_schema.fields["other"]
    assert isinstance(field, Complex)
    assert field.type == "pkg/Sub"
...
def test_parse_default_string_value_with_hash():
...
def test_parse_constant_string_field_double_quotes():
    schema_text = 'string GREETING="hello"\n'
    schema = SchemaRecord(
        id=1,
        name="pkg/msg/StringConstDouble",
        encoding="ros2msg",
        data=schema_text.encode("utf-8"),
    )
    ros2_schema, _ = Ros2MsgSchema().parse(schema)
...
def test_parse_constant_string_field_single_quotes():
    schema_text = "string FAREWELL='bye'\n"
    schema = SchemaRecord(
        id=1,
        name="pkg/msg/StringConstSingle",
        encoding="ros2msg",
        data=schema_text.encode("utf-8"),
    )
    ros2_schema, _ = Ros2MsgSchema().parse(schema)

    field = ros2_schema.fields["FAREWELL"]
    assert isinstance(field, Constant)
    assert field.type == "string"
    assert field.value == "bye"
